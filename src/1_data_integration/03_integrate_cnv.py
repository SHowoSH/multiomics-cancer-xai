# 1_integrate_cnv_data.py
import pandas as pd
import os

print("--- [1단계] CNV 전문가 모델용 데이터 통합 및 변환 시작 ---")
...
CANCER_INFO = {
    'OV':   {'folder': 'TCGA-OV',   'file': 'merged_ov_cnv_cancer(유전자별).csv'},
    'BRCA': {'folder': 'TCGA-BRCA', 'file': 'merged_breast_cnv_cancer.csv'},
    'STAD': {'folder': 'TCGA-STAD', 'file': 'merged_stomach_cnv_cancer.csv'},
    'KIRC': {'folder': 'TCGA-KIRC', 'file': 'merged_kidney_cnv_cancer.csv'},
    'LUSC': {'folder': 'TCGA-LUSC', 'file': 'merged_lusc_cnv_cancer.csv'},
    'LIHC': {'folder': 'TCGA-LIHC', 'file': 'merged_cnv_cancer.csv'}
}

OUTPUT_FILENAME = "cnv_multiclass_basemodel_data.csv"
# -----------
...
for cancer_code, info in CANCER_INFO.items():
    ...
    try:
        ...
        # 1. 필수 컬럼만 선택: 'patient_barcode', 'gene_id', 'copy_number'
        df_subset = df[['patient_barcode', 'gene_id', 'copy_number']].copy()
        
        # 2. 컬럼 이름 표준화
        df_subset.columns = ['patient_barcode', 'gene_id', 'value']
        
        # 3. 유효한 유전자 데이터만 필터링 (ENSG로 시작하는 것만)
        df_subset.dropna(subset=['gene_id'], inplace=True)
        df_subset = df_subset[df_subset['gene_id'].str.startswith('ENSG')]
        
        # 4. 숫자형 데이터 변환 및 결측치(NaN)를 0으로 처리
...
if not long_format_dfs:
    ...
    # 7. Pivot을 사용하여 Long 포맷을 Wide 포맷으로 변환
