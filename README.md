...
> 5가지 종류의 TCGA 다중 오믹스 데이터를 통합하여 5개 암종을 분류하는 고성능 머신러닝 모델을 구축하고, XAI(설명 가능한 인공지능) 기법인 SHAP을 통해 모델의 예측 근거를 분석하여 해석 가능성을 확보한 프로젝트입니다.
...
본 프로젝트의 핵심 목표는 TCGA에서 제공하는 다중 오믹스 데이터를 활용하여 주어진 암 샘플이 어떤 암종(Cancer Type)에 해당하는지 예측하는 고성능 분류 모델을 개발하는 것입니다. 여기서 더 나아가 단순히 예측 결과를 도출하는 것을 넘어 **SHAP (SHapley Additive exPlanations)** 분석을 통해 어떤 오믹스 데이터와 세부 특징(유전자, 메틸레이션 사이트 등)이 모델의 결정에 중요한 영향을 미쳤는지 정량적으로 해석하는 것을 주요 목표로 삼았습니다.

### 1.2. 사용 데이터

-   **데이터 소스**: The Cancer Genome Atlas (TCGA)
-   **분석 대상 암종 (5종류)**: 유방암(BRCA), 위암(STAD), 콩팥암(KIRC), 폐암(LUSC), 간암(LIHC) 
-   **활용 오믹스 데이터 (5종류)**:
    -   유전자 발현 (Gene Expression, RNA-seq)
    -   DNA 메틸레이션 (DNA Methylation)
    -   유전자 변이 (Somatic Mutation)
    -   복제수 변이 (Copy Number Variation, CNV)
    -   miRNA 발현 (miRNA Expression)

---

## 2. 핵심 아키텍처: 계층적 앙상블 모델

본 프로젝트는 **스태킹 앙상블(Stacking Ensemble)** 기법에 기반한 계층적 진단 시스템(Tiered Diagnostic System) 전략을 채택하여 모델의 성능과 안정성을 높였습니다.

<img src="./assets/architecture.png" alt="Architecture Diagram" width="800"/>

-   **Tier 1: 전문가 모델 (Expert Models)**
    5가지 오믹스 데이터 각각에 대해 독립적인 LightGBM 분류 모델을 학습시킵니다. 이를 통해 각 오믹스 데이터가 단독으로 갖는 예측 성능의 베이스라인을 설정하고, 각 데이터의 고유한 특성을 파악합니다.
-   **Tier 3: 메타 모델 (Meta Model)**
    Tier 1 전문가 모델들이 생성한 예측 확률값(Out-of-Fold Predictions)을 새로운 입력 특징(Meta-Features)으로 사용합니다. 이 메타 특징들을 다시 LightGBM 모델로 학습하여 모든 오믹스 정보를 종합적으로 판단하는 최종 모델을 구축합니다.
...
각 오믹스 데이터를 단독으로 사용하여 5-겹 교차 검증을 수행한 결과, **miRNA와 유전자 발현** 데이터가 암종 분류에 가장 강력한 예측 능력을 가진 것을 확인했습니다. 반면, 유전자 변이 데이터는 단독으로 사용 시 가장 낮은 성능을 보였습니다.

| 순위 | 데이터 종류 | 교차 검증 평균 정확도 | 교차 검증 평균 AUC |
|:--:|:---|:---:|:---:|
| 1 | miRNA 발현 | **99.10%** | 0.9988 |
| 2 | 유전자 발현 | 98.67% | 0.9993 |
| 3 | 메틸레이션 | 98.12% | 0.9998 |
| 4 | 복제수 변이 (CNV) | 77.22% | 0.9231 |
| 5 | 유전자 변이 (Mutation) | 47.33% | 0.7755 |

### 3.2. Tier 3: 최종 앙상블 모델 성능

5가지 전문가 모델의 예측을 통합한 최종 메타 모델은 5-겹 교차 검증에서 **평균 정확도 98.67%** 라는 매우 높은 성능을 달성했습니다. 이는 여러 오믹스 정보를 종합함으로써 단일 오믹스 모델의 한계를 보완하고, 더 강건하고 일관된 예측이 가능함을 시사합니다.

| **최종 앙상블 모델 성능** | **[최종 분류 리포트]** |
|:---:|:---:|
| **Accuracy: 98.67%** | ![최종 모델 혼동 행렬](./assets/final_ensemble_confusion_matrix.png) |

---

## 4. 모델 해석 (XAI: Explainable AI)

본 프로젝트의 핵심 중 하나는 SHAP을 이용한 모델 해석입니다. 예측의 'Why'를 설명하기 위해 두 가지 레벨에서 분석을 수행했습니다.

### 4.1. Level 1: 어떤 '특징'이 중요한가? (Feature-level Importance)

각 전문가 모델 내부에서 어떤 유전자, 어떤 메틸레이션 사이트가 예측에 큰 영향을 미쳤는지 분석합니다. 아래는 유전자 발현 모델의 예측에 가장 큰 영향을 준 Top 20 유전자를 시각화한 예시입니다.
...
최종 메타 모델을 분석하여, 5가지 오믹스 데이터(전문가 모델) 중 어떤 것의 예측이 최종 판단에 가장 크게 기여했는지 정량적으로 평가합니다.
